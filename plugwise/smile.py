...
class Smile:
    ...
    def get_device_data(self, dev_id):
        ...
        device_data = self._appliance_data(dev_id)
        ...
        if details["class"] in thermostat_classes:
            ...
            avail_schemas, sel_schema, sched_setpoint = self._schemas(
                details["location"]
            )
            ...
            if self._smile_legacy:
                ...
                device_data["last_used"] = self._last_active_schema(
                    details["location"]
                )
        ...
        if details["class"] in SWITCH_GROUP_TYPES:
            ...
            for member in details["members"]:
                appl_data = self._appliance_data(member)
                if appl_data["relay"]:
                    counter += 1
    ...
    async def set_schedule_state(self, loc_id, name, state):
        ...
        schema_rule_ids = self.__rule_ids_by_name(str(name), loc_id)
    ...
    def _scan_thermostats(self, debug_text="missing text"):
        ...
        self.thermo_locs = self._match_locations()
    ...
    def _temperature_uri(self, loc_id):
        ...
        if self._smile_legacy:
            return self.__temperature_uri_legacy()
    ...
    def _match_locations(self):
        ...
        self.__all_locations()
        self.__all_appliances()
    ...
    def _presets(self, loc_id):
        ...
        if self._smile_legacy:
            return self.__presets_legacy()

        rule_ids = self.__rule_ids_by_tag(tag, loc_id)
        if rule_ids is None:
            rule_ids = self.__rule_ids_by_name("Thermostat presets", loc_id)
            if rule_ids is None:
                return presets
    ...
    def __all_appliances(self):
        ...
        for appliance in self._appliances:
            ...
            if appliance_class in [
                ...
                mod_type = "thermostat"
                module_data = self.___get_module_data(appliance, locator, mod_type)
                appliance_model = self.___check_model(module_data[0])
            ...
            if appliance.find("location") is not None:
                ...
                for appl_type in self.___types_finder(appliance):
    ...
    def __all_locations(self):
        ...
        for location in self._locations:
            ...
            if location_name == "Home":
                ...
                for location_type in self.___types_finder(location):
